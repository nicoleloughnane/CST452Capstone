...
<template>
      ...
      <default-card>
        ...
        <form @submit.prevent="submitForm">
          ...
          <div class="mb-3 xl:w-96">
            ...
            <br />
            <input type="text" class="form-control outline outline-1 outline-brand-darkpurple rounded-md mb-2" v-model.trim="entry.pastor"  />
          </div>
  ...
  export default {
    ...
    methods: {
      ...
      async loadEntry() {
        await api().get(`/sermonnote/${this.entryId}`)
          .then(response => {
      ...
      async updateEntry(formData) {
        await api().put(`/sermonnote/${this.entryId}`, formData)
          .then(response => {
